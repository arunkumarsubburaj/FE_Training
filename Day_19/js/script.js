...
function initializeFn() {
    ...
    employeeData.forEach(function (currentCard) {
...
function filterCards() {
    ...
    console.log(value);
    const cardList = document.querySelectorAll("#employeeDetails .card");
    const cardArrayList = Array.from(cardList);
    const isCardsAvailable = cardArrayList.some(curCard => {
        const empName = curCard.getAttribute("data-employee-name");
        return empName.toLowerCase().indexOf(value) != -1
    })
    cardArrayList.forEach(currentCard => {
        const empName = currentCard.getAttribute("data-employee-name");
        if (empName.toLowerCase().indexOf(value) == -1) {
